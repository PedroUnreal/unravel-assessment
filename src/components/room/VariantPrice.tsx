...
const calculateVariantPrice = (
  ...
  const promoEntry = variantPricing?.promo_list?.find(
    (entry) =>
      typeof entry?.offer_discounted_total_price === 'number' ||
      typeof entry?.offer_total_price === 'number'
  );

  if (promoEntry) {
    const offerTotal = promoEntry.offer_total_price ?? null;
    const offerDiscounted =
      promoEntry.offer_discounted_total_price ?? offerTotal;
    return {
  ...
  if (variantPricing) {
    ...
    return {
      currency: variantCurrency,
      originalPrice:
        hasDiscount && total_price != null ? Math.round(total_price) : null,
      finalPrice:
